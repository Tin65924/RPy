...
function M.py_contains(collection, value)
    ...
    elseif t == "table" then
        if collection._is_set then
            return collection.items[value] == true
        end
...
-- ---------------------------------------------------------------------------
-- Set implementation (Phase 13)
-- ---------------------------------------------------------------------------

function M.py_set_new(iterable)
    local s = { _is_set = true, items = {}, _count = 0 }
    if iterable then
        for _, v in M.py_iter(iterable) do
            M.py_set_add(s, v)
        end
    end
    return s
...
function M.py_set_add(s, item)
    if item == nil then error("set items cannot be nil", 2) end
...
function M.py_set_remove(s, item)
    if not s.items[item] then
        error("KeyError: item not in set", 2)
    end
    s.items[item] = nil
    s._count = s._count - 1
end

function M.py_set_discard(s, item)
...
function M.py_set_clear(s)
    s.items = {}
    s._count = 0
end

function M.py_set_pop(s)
    local k = next(s.items)
    if k == nil then error("KeyError: pop from empty set", 2) end
    s.items[k] = nil
    s._count = s._count - 1
    return k
end
...
-- ---------------------------------------------------------------------------
-- py_iter — generic iterator for for-loops
-- Handles lists (ipairs) and sets (custom).
-- ---------------------------------------------------------------------------

function M.py_iter(iterable)
    ...
    -- Fallback for non-table iterables? (Roblox instances are not handled here)
    return function() end
end
