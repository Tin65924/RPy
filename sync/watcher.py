...
class RPyWatcher(FileSystemEventHandler):
    def __init__(self, src_path: Path, on_changed: Callable[[Path, bool], None]):
    ...
    def on_deleted(self, event):
        ...
        path = Path(event.src_path)
        if path.suffix == ".py":
            self.on_changed(path, True)

def start_watcher(src_path: Path, callback: Callable[[Path, bool], None]):
